...
import { createAd, getAds, processAdInvestment, generateUUID, getGlobalSettings, promoteAdCampaign } from '../services/storageService';
...
export const AdCampaignPage: React.FC<AdCampaignPageProps> = ({ currentUser, refreshUser, onNavigate }) => {
  ...
  const [bidAmount, setBidAmount] = useState<number>(0.50); // Valor do lance (CPM)
  const [billingCycle, setBillingCycle] = useState<'DAILY' | 'WEEKLY'>('WEEKLY');
  const [isAutoRenew, setIsAutoRenew] = useState(true);
  const [promotionDays, setPromotionDays] = useState<number>(0);
  ...
  const costPer1kReach = bidAmount;
  ...
  const promotionCost = useMemo(() => {
    return promotionDays * (settings?.promotedCarouselMinBidPerDay || 500);
  }, [promotionDays, settings?.promotedCarouselMinBidPerDay]);

  const GrandTotal = calculatedTotalBudget + promotionCost;
  ...
  const hasBalance = (currentUser.balance || 0) >= GrandTotal;
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      // Se o usuário selecionou promoção no carrossel, processa o pagamento extra
      if (promotionDays > 0) {
          const costPerDay = settings?.promotedCarouselMinBidPerDay || 500;
          await promoteAdCampaign(currentUser.id, newAd.id, promotionDays, costPerDay);
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#080a0f] pt-16 md:pt-24 pb-20 md:pb-32 px-2 md:px-4">
      <div className="max-w-7xl mx-auto">
        
        <div className="bg-white dark:bg-darkcard p-5 md:p-8 rounded-[2rem] md:rounded-[2.5rem] shadow-xl border border-blue-500/10 mb-6 md:mb-10 relative overflow-hidden group">
           ...
           <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6 relative z-10">
              <div className="flex-1">
                 <h3 className="text-lg md:text-xl font-black dark:text-white uppercase tracking-tighter flex items-center gap-2">
                    <SparklesIcon className="h-5 w-5 md:h-6 md:w-6 text-yellow-500" /> 
                    Prioridade de Relevância
                 </h3>
                 <p className="text-[9px] md:text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">
                    Nossos algoritmos priorizam conteúdos com maior investimento.
                 </p>
              </div>
              <div className="bg-blue-50 dark:bg-blue-900/10 p-3 md:p-4 rounded-2xl border border-blue-100 dark:border-blue-500/20 max-w-full md:max-w-xs">
                 <p className="text-[8px] md:text-[9px] text-blue-800 dark:text-blue-300 font-medium leading-relaxed">
                    <strong>Segmentação Precisa:</strong> Apareça rigorosamente para as audiências selecionadas.
                 </p>
              </div>
           </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 md:mb-10 gap-4 md:gap-6">
          <div>
            <h1 className="text-3xl md:text-5xl font-black text-gray-900 dark:text-white tracking-tighter flex items-center gap-2 md:gap-3">
              <RocketLaunchIcon className="h-8 w-8 md:h-10 md:w-10 text-blue-600" /> Impulsionar
            </h1>
            <p className="text-gray-500 dark:text-gray-400 font-bold uppercase text-[9px] md:text-[10px] tracking-widest mt-1">Lances maiores garantem o topo do Feed</p>
          </div>
          
          <div className="flex items-center gap-3 md:gap-4 bg-white dark:bg-darkcard p-4 md:p-5 rounded-[1.5rem] md:rounded-[2.5rem] shadow-xl border dark:border-white/5">
             <div className="text-right">
                <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Saldo Atual</p>
                <p className={`text-xl md:text-2xl font-black ${hasBalance ? 'text-blue-600' : 'text-red-500'}`}>{formatCurrency(currentUser.balance || 0)}</p>
             </div>
             <button onClick={() => onNavigate?.('profile')} className="p-2 sm:p-3 bg-blue-50 dark:bg-blue-600/10 text-blue-600 rounded-xl sm:rounded-2xl hover:bg-blue-600 hover:text-white transition-all shadow-md">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-10">
          <main className="lg:col-span-8 space-y-6 md:space-y-8">
             <div className="flex justify-between mb-4 px-1 md:px-2 gap-1 overflow-x-auto no-scrollbar pb-2">
                {['objective', 'creative', 'targeting', 'budget'].map((s, idx) => (
                  <div key={s} className="flex flex-col items-center gap-2 flex-1 min-w-[60px] relative">
                     <div className={`w-10 h-10 md:w-12 md:h-12 rounded-[1rem] md:rounded-[1.2rem] flex items-center justify-center font-black text-xs md:text-sm z-10 transition-all ${currentStep === s ? 'bg-blue-600 text-white shadow-xl scale-110' : idx < ['objective', 'creative', 'targeting', 'budget'].indexOf(currentStep) ? 'bg-green-500 text-white' : 'bg-gray-200 dark:bg-white/10 text-gray-400'}`}>
                        {idx < ['objective', 'creative', 'targeting', 'budget'].indexOf(currentStep) ? <CheckCircleIcon className="h-5 w-5 md:h-6 md:w-6" /> : idx + 1}
                     </div>
                     {idx < 3 && <div className={`absolute top-5 md:top-6 left-1/2 w-full h-1 -z-0 ${idx < ['objective', 'creative', 'targeting', 'budget'].indexOf(currentStep) ? 'bg-green-500/30' : 'bg-gray-100 dark:bg-white/5'}`}></div>}
                     <span className={`text-[8px] font-black uppercase tracking-tighter ${currentStep === s ? 'text-blue-600' : 'text-gray-400'}`}>
                        {s === 'objective' ? 'Objetivo' : s === 'creative' ? 'Mídia' : s === 'targeting' ? 'PúblicO' : 'OrçamentO'}
                     </span>
                  </div>
                ))}
             </div>

             <div className="bg-white dark:bg-darkcard rounded-[2rem] md:rounded-[3.5rem] p-5 md:p-12 shadow-2xl border dark:border-white/5 min-h-[400px] md:min-h-[550px] flex flex-col">
                {currentStep === 'objective' && (
                  <div className="animate-fade-in space-y-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-6">
                       {[
                         { id: 'engagement', label: 'Seguidores/Fãs', desc: 'Foco em ganhar novos fãs e audiência.', icon: UserGroupIcon },
                         ...
                         <button 
                           ...
                           className={`p-5 md:p-10 rounded-[2rem] md:rounded-[3rem] border-4 transition-all text-left flex flex-col gap-3 md:gap-5 group ${objective === obj.id ? 'border-blue-600 bg-blue-50 dark:bg-blue-600/10 shadow-2xl' : 'border-gray-50 dark:border-white/5 hover:border-gray-200 dark:hover:border-white/10'}`}
                         >
                            <div className={`p-4 md:p-5 rounded-[1.2rem] md:rounded-[1.5rem] w-fit shadow-lg ${objective === obj.id ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-white/5 text-gray-400 group-hover:text-blue-600'}`}>
                               <obj.icon className="h-6 w-6 md:h-8 md:w-8" />
                            </div>
                            <div>
                               <h4 className="font-black dark:text-white uppercase text-[10px] md:text-xs tracking-widest">{obj.label}</h4>
                               <p className="text-[10px] md:text-[11px] text-gray-500 dark:text-gray-400 mt-1 md:mt-2 leading-relaxed font-medium">{obj.desc}</p>
                            </div>
                         </button>
                       ))}
                    </div>
                    <div className="pt-6 md:pt-10 flex justify-end">
                       <button onClick={() => setCurrentStep('creative')} className="w-full sm:w-auto bg-blue-600 text-white px-10 md:px-12 py-4 md:py-5 rounded-2xl font-black text-xs md:text-sm uppercase flex items-center justify-center gap-3 shadow-xl active:scale-95 transition-all">Próximo <ChevronRightIcon className="h-5 w-5"/></button>
                    </div>
                  </div>
                )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10">
                       <div className="space-y-6">
                          ...
                          <div className="space-y-2">
                             ...
                             <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Diga aos usuários por que eles devem te seguir..." className="w-full p-5 bg-gray-50 dark:bg-white/5 rounded-3xl dark:text-white outline-none focus:ring-2 focus:ring-blue-600 font-medium h-40 resize-none shadow-inner" />
                          </div>
                          <div className="space-y-2">
                ...
                {currentStep === 'targeting' && (
                   <div className="animate-fade-in space-y-10">
                      ...
                      <div className="space-y-10">
                        {/* AGE SLIDER */}
                        <div className="bg-gray-50 dark:bg-white/5 p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-gray-100 dark:border-white/10 space-y-6 relative overflow-hidden">
                           <div className="absolute top-0 right-0 p-3 md:p-4 bg-green-500/10 text-green-600 rounded-bl-2xl">
                              <span className="text-[8px] md:text-[9px] font-black uppercase tracking-widest flex items-center gap-1"><ShieldCheckIcon className="h-3 w-3" /> Filtro Ativo</span>
                           </div>
                           <div className="flex items-center gap-3">
                              <UserIcon className="h-5 w-5 md:h-6 md:w-6 text-blue-600" />
                              <h4 className="font-black dark:text-white uppercase text-[10px] md:text-xs tracking-widest">Faixa Etária (Obrigatório)</h4>
                           </div>
                           <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8">
                              <div className="space-y-3">
                                 <div className="flex justify-between">
                                    <label className="text-[9px] md:text-[10px] font-black text-gray-400 uppercase">Idade Mínima</label>
                                    <span className="text-[10px] md:text-xs font-black text-blue-600">{minAge} anos</span>
                                 </div>
                              ...
                              <div className="space-y-3">
                                 <div className="flex justify-between">
                                    <label className="text-[9px] md:text-[10px] font-black text-gray-400 uppercase">Idade Máxima</label>
                                    <span className="text-[10px] md:text-xs font-black text-blue-600">{maxAge} anos</span>
                                 </div>
                ...
                {currentStep === 'budget' && (
                  <div className="animate-fade-in space-y-8 md:space-y-10">
                    <div className="text-center max-w-lg mx-auto">
                       <h2 className="text-xl md:text-3xl font-black dark:text-white tracking-tight uppercase">Investimento</h2>
                       <p className="text-[10px] md:text-sm text-gray-500 font-medium mt-2 leading-tight">Quanto maior o lance, maior o topo.</p>
                    </div>
                       ...
                       <div className="space-y-8 md:space-y-12">
                          <div className="space-y-5">
                             <div className="flex justify-between items-center">
                                <label className="text-[9px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest">Lance de Leilão (CPM)</label>
                                <span className={`px-4 py-1.5 rounded-xl text-xs font-black shadow-sm ${bidAmount > 1.0 ? 'bg-purple-50 text-purple-600' : 'bg-blue-50 text-blue-600'}`}>
                                   {formatCurrency(bidAmount)} / 1k imp.
                                </span>
                             </div>
                             <input type="range" min={0.10} max={10.00} step={0.10} value={bidAmount} onChange={e => setBidAmount(parseFloat(e.target.value))} className="w-full h-2 bg-gray-100 dark:bg-white/10 rounded-full appearance-none cursor-pointer accent-blue-600" />
                             <div className="flex justify-between text-[8px] font-bold text-gray-400 uppercase">
                                <span>Econômico</span>
                                <span className="text-purple-500">Autoridade Máxima</span>
                             </div>
                          </div>

                          <div className="space-y-5">
                             <div className="flex justify-between items-center">
                                <label className="text-[9px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest">Investimento Inicial</label>
                          ...
                          <div className="space-y-5">
                             <div className="flex justify-between items-center">
                                <label className="text-[9px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest">Força de Alcance (Impacto Diário)</label>
                          ...
                          <div className="space-y-5">
                             <div className="flex justify-between items-center">
                                <label className="text-[9px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest">Frequência de Renovação</label>
                                <div className="flex gap-2">
                                   <button 
                                     onClick={() => setBillingCycle('DAILY')}
                                     className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase transition-all ${billingCycle === 'DAILY' ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-100 dark:bg-white/10 text-gray-400'}`}
                                   >
                                     Diário
                                   </button>
                                   <button 
                                     onClick={() => setBillingCycle('WEEKLY')}
                                     className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase transition-all ${billingCycle === 'WEEKLY' ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-100 dark:bg-white/10 text-gray-400'}`}
                                   >
                                     Semanal
                                   </button>
                                </div>
                             ...
                             {isAutoRenew && (
                               <div className="flex items-center gap-2 px-4 py-2 bg-green-500/10 border border-green-500/20 rounded-xl animate-pulse">
                                  ...
                                  <p className="text-[9px] text-green-600 font-black uppercase tracking-tight">
                                     Será debitado {formatCurrency(renewalAmount)} a cada {billingCycle === 'WEEKLY' ? '7 dias' : 'dia'}.
                                  </p>
                               </div>
                             )}
                             ...
                             <div className="space-y-5 pt-4">
                                <label className="text-[9px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                                   <RocketLaunchIcon className="h-4 w-4 text-blue-600" /> Promover no Carrossel (Topo do Feed)
                                </label>
                                <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                                   {[1, 2, 5, 7, 15, 30].map(days => (
                                     <button
                                       type="button"
                                       key={days}
                                       onClick={() => setPromotionDays(promotionDays === days ? 0 : days)}
                                       className={`py-3 px-1 rounded-xl border-2 flex flex-col items-center justify-center gap-0.5 transition-all ${promotionDays === days ? 'bg-blue-600 border-blue-600 text-white shadow-lg scale-105' : 'bg-gray-50 dark:bg-white/5 border-transparent text-gray-500'}`}
                                     >
                                        <span className="text-sm font-black">{days}</span>
                                        <span className="text-[7px] font-black uppercase">DIAS</span>
                                     </button>
                                   ))}
                                </div>
                                {promotionDays > 0 && (
                                   <div className="p-4 bg-blue-50 dark:bg-blue-900/10 rounded-2xl border border-blue-500/20 flex gap-3 animate-fade-in">
                                      <StarIcon className="w-5 h-5 text-blue-600 shrink-0" />
                                      <p className="text-[9px] text-blue-800 dark:text-blue-300 font-black leading-tight uppercase">
                                         Custo extra: {formatCurrency(promotionDays * (settings?.promotedCarouselMinBidPerDay || 500))}
                                      </p>
                                   </div>
                                )}
                             </div>
                        ...
                        <div className="bg-gray-50 dark:bg-black/40 p-6 md:p-10 rounded-[2.5rem] md:rounded-[4rem] border-2 border-dashed border-gray-200 dark:border-white/10 flex flex-col justify-center items-center text-center shadow-inner relative overflow-hidden">
                          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 relative z-10">Investimento Total</p>
                          <p className="text-5xl md:text-7xl font-black text-gray-900 dark:text-white tracking-tighter relative z-10">{formatCurrency(GrandTotal)}</p>
                          <div className="mt-8 flex items-center gap-3 text-green-600 bg-green-50 dark:bg-green-900/20 px-6 py-2.5 rounded-2xl relative z-10 border border-green-100 dark:border-green-900/30">
                             ...
                             <span className="text-[10px] font-black uppercase tracking-widest">Est: +{(dailyReachGoal * durationDays / 100).toFixed(0)} conexões</span>
                          </div>
                        </div>
                    </div>

                    <div className="pt-8 md:pt-10 flex flex-col items-end gap-6 border-t dark:border-white/5">
                       {!hasBalance && (
                         <div className="flex items-center gap-3 md:gap-4 bg-red-50 dark:bg-red-900/20 p-5 md:p-6 rounded-[1.5rem] md:rounded-[2rem] border-2 border-red-100 dark:border-red-500/30 w-full animate-pulse">
                            <ExclamationTriangleIcon className="h-6 w-6 md:h-8 md:w-8 text-red-600 shrink-0" />
                            <p className="text-[11px] md:text-sm text-red-700 dark:text-red-400 font-bold uppercase leading-tight">Saldo insuficiente para este nível de alcance.</p>
                         </div>
                       )}
                       <div className="flex flex-col sm:flex-row justify-between w-full gap-4">
                          <button onClick={() => setCurrentStep('targeting')} className="text-gray-400 font-black text-[10px] md:text-xs uppercase flex items-center justify-center sm:justify-start gap-2 hover:text-gray-900 transition-colors py-2"><ChevronLeftIcon className="h-4 w-4" /> Voltar</button>
                          <button 
                            onClick={handlePublish}
                            disabled={loading || !hasBalance || !imageUrl || !title}
                            className={`w-full sm:w-auto px-10 md:px-16 py-5 md:py-6 rounded-2xl md:rounded-[2rem] font-black text-sm md:text-lg uppercase flex items-center justify-center gap-4 md:gap-5 shadow-2xl transition-all active:scale-95 ${loading || !hasBalance || !imageUrl || !title ? 'bg-gray-300 cursor-not-allowed text-gray-500' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
                          >
                             {loading ? <div className="w-6 h-6 md:w-8 md:h-8 border-4 border-white border-t-transparent animate-spin rounded-full" /> : <><BanknotesIcon className="h-6 w-6 md:h-8 md:w-8"/> ATIVAR DESTAQUE</>}
                          </button>
                       </div>
                    </div>
                  </div>
                ...
                <div className="bg-white dark:bg-darkcard rounded-[3rem] shadow-2xl border border-gray-100 dark:border-white/10 overflow-hidden transform transition-all duration-500 max-w-[380px] mx-auto group">
                   ...
                   <div className="p-6">
                      ...
                      <p className="text-[11px] text-gray-700 dark:text-gray-300 font-medium leading-relaxed line-clamp-3">
                        {description || 'Sua mensagem persuasiva para atrair novos seguidores aparecerá aqui.'}
                      </p>
